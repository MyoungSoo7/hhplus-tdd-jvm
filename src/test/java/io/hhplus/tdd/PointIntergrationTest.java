...
public class PointIntergrationTest {
    ...
    void 포인트_충전_이용_내역_조회() {
        ...
        // when
        List<PointHistory> userHistory = pointService.selectUserPointHistory(userId);
    ...
    void 포인트_이용() {
        ...
        pointService.chargeUserPoint(userId, defaultAmount);
    ...
    void 여러_건의_포인트_충전_이용_요청이_올_경우_순차적으로_처리되어야_한다() throws InterruptedException {
        ...
        pointService.chargeUserPoint(userId, startAmount);
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    if (taskNum % 2 == 0) {
                        pointService.chargeUserPoint(userId, chargeAmount);// 충전
                    } else {
                        pointService.useUserPoint(userId, useAmount); // 사용
                    }
                } finally {
    ...
    void 여러_건의_포인트_사용_요청이_올_경우_올바른_금액이_사용_되어야_한다() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try {
                    pointService.useUserPoint(userId, useAmount);// 충전
                } finally {
                    countDownLatch.countDown();
                }
            });
